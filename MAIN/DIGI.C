...
#define _LOOPING DSBPLAY_LOOPING
...
typedef struct sound_object {
	...
	LPDIRECTSOUNDBUFFER			handle;			// What handle this sound is playing on.  Valid only if SOF_PLAYING is set.
...
void digi_close_digi() {
	IDirectSound_Release(sound_device);
	digi_driver_board = 0;
}
...
int digi_init_digi() {
	if (DirectSoundCreate8(NULL, &sound_device, NULL) != DS_OK) {
		Error("Could not create sound device");
	}
	if (IDirectSound8_SetCooperativeLevel(sound_device, hwnd, DSSCL_PRIORITY) != DS_OK) {
		Error("Could not set sound cooperative level");
	}
...
static LPDIRECTSOUNDBUFFER SampleHandles[32] = { 0xffff, 0xffff, 0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff,0xffff };
...
void digi_reset_digi_sounds()
{
	...
	for (i=0; i<32; i++ )	{
		if (SampleHandles[i] < _MAX_VOICES && SampleHandles[i])	{
			IDirectSoundBuffer8_GetStatus(SampleHandles[i], &status);
			if ( status & DSBSTATUS_PLAYING  )		{
				//mprintf(( 0, "Stopping sound %d.\n", next_handle ));
				IDirectSoundBuffer8_Stop(SampleHandles[i]);
			}
...
// TODO: Maybe make errors less harsh, since sounds are not a deal-breaker
LPDIRECTSOUNDBUFFER digi_start_sound(_SOS_START_SAMPLE * sampledata, short soundnum) {
	if (!Digi_initialized) return 0xFFFF;

	WAVEFORMATEX wfx;
	DSBUFFERDESC bufferdesc;
	LPDIRECTSOUNDBUFFER ds_buffer = NULL;
	VOID *pvAudioPtr1 = NULL, *pvAudioPtr2 = NULL;
	DWORD dwAudioBytes1 = 0, dwAudioBytes2 = 0;
	LONG lPan, lVolume, lLooping = 0;

	// This is the wave format used in Descent
	...
	bufferdesc.dwFlags = DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME | DSBCAPS_GLOBALFOCUS;
	bufferdesc.dwBufferBytes = sampledata->dwSampleSize;
	bufferdesc.lpwfxFormat = &wfx;

	// Create secondary sound buffer
	if (IDirectSound8_CreateSoundBuffer(sound_device, &bufferdesc, &ds_buffer, NULL) != DS_OK) {
		Error("Could not create secondary sound buffer");
	}

	// Save this stuff
	SampleHandles[soundnum] = ds_buffer;
	SoundNums[soundnum] = soundnum;
	SoundVolumes[soundnum] = sampledata->wVolume;

	// Lock the sound buffer
	if (IDirectSoundBuffer8_Lock(ds_buffer, 0, sampledata->dwSampleSize, &pvAudioPtr1, &dwAudioBytes1, &pvAudioPtr2, &dwAudioBytes2, 0) != DS_OK) {
		Error("Could not lock sound buffer");
	}

	// Convert pan location from Descent style to DS style
	lPan = ((sampledata->wSamplePanLocation * (DSBPAN_RIGHT - DSBPAN_LEFT)) / 0xffff) + DSBPAN_LEFT;

	// Set the pan
	if (IDirectSoundBuffer8_SetPan(ds_buffer, lPan) != DS_OK) {
		Error("Could not set sound buffer pan");
	}

	// Convert volume from Descent style to DS style (kind of hacked, otherwise sounds are almost silent)
	lVolume = ((sampledata->wVolume + _DIGI_MAX_VOLUME) * (DSBVOLUME_MAX - DSBVOLUME_MIN)) / 0x7fff + DSBVOLUME_MIN;

	// Clip volume to bounds; self-destruct sounds causing crashes
	...
	// Set the volume
	if (IDirectSoundBuffer8_SetVolume(ds_buffer, lVolume) != DS_OK) {
		Error("Could not set sound buffer volume");
	}

	// Copy sample data to sound buffer
	ubyte *ptr = sampledata->lpSamplePtr;
	memcpy(pvAudioPtr1, ptr, dwAudioBytes1);
	ptr += dwAudioBytes1;
	memcpy(pvAudioPtr2, ptr, dwAudioBytes2);

	// Unlock the sound buffer
	if (IDirectSoundBuffer8_Unlock(ds_buffer, pvAudioPtr1, dwAudioBytes1, pvAudioPtr2, dwAudioBytes2) != DS_OK) {
		Error("Could not unlock sound buffer");
	}

	lLooping = sampledata->wSampleFlags & _LOOPING;

	// Play the buffer!
	if (IDirectSoundBuffer8_Play(ds_buffer, 0, 0, lLooping) != DS_OK) {
		Error("Could not play sound buffer");
	}
...
int digi_is_sound_playing(int soundno)
{
	...
	IDirectSoundBuffer8_GetStatus(SampleHandle, &status);
	if ( (SampleHandle < _MAX_VOICES) && (status & DSBSTATUS_PLAYING) )
		return 1;
	return 0;
}
...
void digi_init_sounds()
{
	...
	for (i=0; i<MAX_SOUND_OBJECTS; i++ )	{
		if (digi_sounds_initialized) {
			if ( SoundObjects[i].flags & SOF_PLAYING && SoundObjects[i].handle )	{
				IDirectSoundBuffer8_Stop( SoundObjects[i].handle );
			}
		}
	...
	for (i = 0; i < MAX_SOUND_OBJECTS; i++) {
		if (SoundObjects[i].flags & SOF_USED) {
			...
			if (!(SoundObjects[i].flags & SOF_PLAY_FOREVER)) {
				// Check if its done.
				if (SoundObjects[i].flags & SOF_PLAYING && SoundObjects[i].handle) {
					IDirectSoundBuffer8_GetStatus(SoundObjects[i].handle, &status);
					if (status & DSBSTATUS_PLAYING) {
						SoundObjects[i].flags = 0;	// Mark as dead, so some other sound can use this sound
			...
			if (SoundObjects[i].flags & SOF_LINK_TO_POS) {
				...
				if ((objp->type == OBJ_NONE) || (objp->signature != SoundObjects[i].objsignature)) {
					...
					if ((SoundObjects[i].flags & SOF_PLAYING) && (SoundObjects[i].flags & SOF_PLAY_FOREVER) && SoundObjects[i].handle) {
						IDirectSoundBuffer8_Stop(SoundObjects[i].handle);
					}
					SoundObjects[i].flags = 0;	// Mark as dead, so some other sound can use this sound
					continue;		// Go on to next sound...
			...
			if (oldvolume != SoundObjects[i].volume) {
				if (SoundObjects[i].volume < 1) {
					...
					else if (SoundObjects[i].handle) {
						lVolume = ((SoundObjects[i].volume) * (DSBVOLUME_MAX - DSBVOLUME_MIN)) / 0x7fff + DSBVOLUME_MIN;
						IDirectSoundBuffer8_SetVolume(SoundObjects[i].handle, fixmuldiv(lVolume, digi_volume, F1_0));
					}
				}
			}
			...
			if (oldpan != SoundObjects[i].pan) {
				if (SoundObjects[i].flags & SOF_PLAYING && SoundObjects[i].handle) {
					lPan = ((SoundObjects[i].pan * (DSBPAN_RIGHT - DSBPAN_LEFT)) / 0xffff) + DSBPAN_LEFT;
					IDirectSoundBuffer8_SetPan(SoundObjects[i].handle, lPan);
				}
			}
