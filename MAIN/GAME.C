...
void game_init_render_buffers(int screen_mode, int render_w, int render_h, int use_paging, int render_method, int compatible_menus )
{
	if (!VR_offscreen_buffer)	{
		...
		if (gr_video_memory == NULL) { 
			Error("Could not initialize window buffer.");
		}
...
void render_countdown_gauge()
{
	...
	fix scale_y = fixdiv(i2f(grd_curscreen->sc_h), i2f(200));
	fix scale_factor = min(scale_x, scale_y);
	...
	if ((Newdemo_state == ND_STATE_PLAYBACK) || (Newdemo_state == ND_STATE_RECORDING)) {
		...
		if (Newdemo_state == ND_STATE_PLAYBACK) {
			if (Newdemo_vcr_state != ND_STATE_PRINTSCREEN) {
			  	sprintf_s(message, 128, "%s (%d%%%% %s)", TXT_DEMO_PLAYBACK, newdemo_get_percent_done(), TXT_DONE);
			} else {
				sprintf_s(message, 128, "");
			}
		} else 
...
void ReadControls() {
	...
	while ((key = key_inkey_time(&key_time)) != 0)	{
		...
		case KEY_9:
		case KEY_0:
			// Hack since keys are mapped different than DOS, otherwise can't pick Mega Missile
			if (key == KEY_0) {
				key = KEY_9 + 1;
			}
...
void GameLoop(int RenderFlag, int ReadControlsFlag)
{
	...
	if (last_start > 0) {
		long sleep_time = 17 - (timer_get_millis() - last_start);
		if (sleep_time > 0) {
			Sleep(sleep_time);
		}
	}
	last_start = timer_get_millis();
