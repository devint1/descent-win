...
static int get_event(hmp_file *hmp, event *ev) {
	...
	if (ev_num < 0xf0) {
		...
		if (cmdlen[((ev_num) >> 4) - 8] == 3) {
			ev->msg[2] = *(trk->cur++);
			trk->left--;
			if (ev->msg[0] >> 4 == 0xB && ev->msg[1] == 0x07) {
				channel_volume[ev->msg[0] & 0xf] = ev->msg[2];
				ev->msg[2] = ev->msg[2] * midi_volume / 128;
			}
		}
	}
	else if (ev_num == 0xff) {
...
void hmp_setvolume(hmp_file *hmp, int volume) {
	if (hmp)
		for (int channel = 0; channel < 16; channel++)
			midiOutShortMsg((HMIDIOUT)hmp->hmidi, (DWORD)(channel | 0xB << 4 | 0x07 << 8 | (channel_volume[channel] * volume / 128) << 16));
