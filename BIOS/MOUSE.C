...
#include "timer.h"
#include "i86.h"
#include "GR.H" // Needed to get center pos
...
void mouse_handler (UINT uMsg, WPARAM wParam, LPARAM lParam) {
	Mouse.ctime = timer_get_fixed_secondsX();
	
	// Wheel or button message?
	bool wheel = false;
	switch(uMsg) {
	case WM_MOUSEHWHEEL:
	case WM_MOUSEWHEEL:
	...
	if (!wheel) {
		if (uMsg == WM_LBUTTONDOWN) {	// left button pressed
		...
		else if (uMsg == WM_LBUTTONUP) {  // left button released
			if (Mouse.pressed[MB_LEFT]) {
		...
		if (uMsg == WM_RBUTTONDOWN) {	// right button pressed
		...
		else if (uMsg == WM_RBUTTONUP) {// right button released
			if (Mouse.pressed[MB_RIGHT]) {
		...
		if (uMsg == WM_MBUTTONDOWN) { // middle button pressed
		...
		else if (uMsg == WM_MBUTTONUP) { // middle button released
			if (Mouse.pressed[MB_MIDDLE]) {
...
int mouse_init(int enable_cyberman)
{
   ...
   /*if (_dos_getvect(0x33) == NULL) {
	...
	Mouse.num_buttons = 3;// outregs.w.bx;
	Mouse.cyberman = 0;
	...
	center.x = grd_curscreen->sc_w / 2;
	center.y = grd_curscreen->sc_h / 2;
