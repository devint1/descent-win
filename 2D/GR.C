...
extern LPBITMAPINFO bmi;
extern HDC hDibDC;
CRITICAL_SECTION palettecs;
...
int gr_set_mode(int w, int h) {
	...
	if (!window_installed) {
		...
		// Try to go into full screen
		if (!FindArg("-windowed")) {
			DEVMODE newSettings;
			EnumDisplaySettings(NULL, 0, &newSettings);
			newSettings.dmPelsWidth = w;
			newSettings.dmPelsHeight = h;
			if (ChangeDisplaySettings(&newSettings, CDS_FULLSCREEN) == DISP_CHANGE_SUCCESSFUL) {
				fullscreen_installed = 1;
			}
			else {
				fullscreen_installed = 0;
			}
		}

		// Create render window thread
		CreateThread(NULL, 0, render_window_run, NULL, 0, NULL);
