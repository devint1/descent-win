...
void __interrupt __far key_handler(WPARAM wParam, LPARAM lParam) {
	...
	// Distinguih between left/right keys
	switch (scancode) {
	case 16:
		if ((lParam & (1 << 24)) >> 24) scancode = VK_RSHIFT;
		else scancode = VK_LSHIFT;
		break;
	case 17:
		if ((lParam & (1 << 24)) >> 24) scancode = VK_RCONTROL;
		else scancode = VK_LCONTROL;
		break;
	case 18:
		if ((lParam & (1 << 24)) >> 24) scancode = VK_RMENU;
		else scancode = VK_LMENU;
		break;
	}

	// Extract transition state from lParam
	breakbit = (lParam & (1 << 31)) >> 31;
