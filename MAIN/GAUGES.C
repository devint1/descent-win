...
void hud_show_score()
{
	...
	if ( ((Game_mode & GM_MULTI) && !(Game_mode & GM_MULTI_COOP)) ) {
		sprintf_s(score_str, 20, "%s: %5d", TXT_KILLS, Players[Player_num].net_kills_total);
	} else {
		sprintf_s(score_str, 20, "%s: %5d", TXT_SCORE, Players[Player_num].score);
	}
...
//	#endif
	...
	switch (Primary_weapon) {
		case 0:
			if (Players[Player_num].flags & PLAYER_FLAGS_QUAD_LASERS)
				sprintf_s(weapon_str, 32, "%s %s %i", TXT_QUAD, TXT_LASER, Players[Player_num].laser_level+1);
			else
				sprintf_s(weapon_str, 32, "%s %i", TXT_LASER, Players[Player_num].laser_level+1);
			break;
		case 1:
			sprintf_s(weapon_str, 32, "%s: %i", TXT_W_VULCAN_S, f2i(Players[Player_num].primary_ammo[Primary_weapon] * VULCAN_AMMO_SCALE));
			break;
		case 2:
			strcpy_s(weapon_str, 32, TXT_W_SPREADFIRE_S);
			break;
		#ifndef SHAREWARE
		case 3:
			strcpy_s(weapon_str, 32, TXT_W_PLASMA_S);
			break;
		case 4:
			strcpy_s(weapon_str, 32, TXT_W_FUSION_S);
			break;
		#endif
	}
	...
	switch (Secondary_weapon) {
		case 0:	strcpy_s(weapon_str, 32, TXT_CONCUSSION);	break;
		case 1:	strcpy_s(weapon_str, 32, TXT_HOMING);	break;
		case 2:	strcpy_s(weapon_str, 32, TXT_PROXBOMB   );	break;
		#ifndef SHAREWARE
		case 3:	strcpy_s(weapon_str, 32, TXT_SMART);	break;
		case 4:	strcpy_s(weapon_str, 32, TXT_MEGA);	break;
		#endif
