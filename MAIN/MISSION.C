...
int build_mission_list(int anarchy_mode)
{
	...
	if( fhandle = FindFirstFile( "*.MSN", &find ) != INVALID_HANDLE_VALUE )	{
		do	{
			FILE *mfile;
			int is_anarchy;
			char temp[13],*t;

			strcpy(temp,find.cFileName);
		...
		} while( !FindNextFile( fhandle, &find ) && count<MAX_MISSIONS);
	}
#ifdef USE_CD
