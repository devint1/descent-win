...
get_char_width(unsigned int c,unsigned int c2,int *width,int *spacing)
{
	unsigned int letter;
...
void read_font(grs_font *font, CFILE *fontfile) {
	...
	font->ft_data = ft_data;
	font->ft_chars = ft_chars;
	font->ft_widths = ft_widths;
	font->ft_kerndata = ft_kerndata;
}
...
grs_font * gr_init_font( char * fontname )
{
	...
	if (font->ft_flags & FT_PROPORTIONAL) {
		...
		font->ft_widths = (short *) (((int) font->ft_widths) + FONT_HEADER_OFFSET + ((ubyte *) font));

		font->ft_data = ((int) font->ft_data) + FONT_HEADER_OFFSET + ((ubyte *) font);
	...
	if (font->ft_flags & FT_KERNED) 
		font->ft_kerndata = ((int) font->ft_kerndata) + FONT_HEADER_OFFSET + ((ubyte *) font);
